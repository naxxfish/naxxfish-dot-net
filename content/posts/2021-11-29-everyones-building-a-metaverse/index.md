...
Now imagine that it's not just transactions on that chain, it's entire 3D worlds and avatars. Every client would have to download the chain *in full* and subsequently keep it up to date with all published content, even if they were never going to use it. And nothing could ever be deleted from it, so it would be a huge chunk of data that would just continue rapidly expanding forever until it became too large to move from client to client in a practical way. 
...
This definitely works in principle - if you own an avatar that looks like yourself, you don't want other people using it without your permission - and being able to prove you own it might help.  First, it relies on the client be trusted not to render that avatar for other people trying to use it without owning it. Secondly, what would stop someone from copying an avatar and tweaking it in a non-material way such that it appears identical but does not have the same ownership restrictions and can be freely used without triggering the ownership checks?
